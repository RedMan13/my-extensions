...
export class Parser {
    ...
    static blockInputs = {
        ...
        unknown:   /^\[!\s*(?<default>\$[_a-z][_$a-z0-9]*|(?:[^\]]|\\\])*)\s*\]/i,
    ...
    parseBlockDef() {
        ...
        for (; true; i++) {
            ...
            if (/^(::|\n)/.test(this.str.slice(i)) || !this.str.length) break;
    ...
    parse() {
        ...
        while (this.str.length) {
            ...
            if (typeof (match ?? '') === 'string') return `${match} (@char ${this.src.length - this.str.length})`;
            members.push(match);
 ...
 */
/**
 * @typedef {[...(string|{ type: string, [key: string]: string }), string[]]} ToolboxBlock
 */
/**
 * @typedef {{ [key: string]: string | string[] }} Variables
 */
/**
 * @typedef {{
 *   [key: (string|number)]: (Category | Label | ToolboxBlock | Variables),
 *   vars: Variables
 * }} Toolbox
 */
export const tokenHandles = {
    ...
    label([onClick, label], gen) { gen.push({ label, onClick }); gen.vars[onClick] = () => {}; },
    block(block, gen) {
        const type = 'type;' + block.map(arg => typeof arg === 'string' ? arg : `{${arg.type}}`).join(' ');
        block.at(-1).push(type);
        gen.push(block);
    }
}
/**
